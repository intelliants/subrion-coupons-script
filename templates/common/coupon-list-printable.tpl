<div id="coupon-list-{$coupon.id}" class="media ia-item ia-item-bordered ia-item-coupon{if $coupon.featured} ia-item-featured{/if}{if $coupon.sponsored} ia-item-sponsored{/if}" data-affiliate-link="{if $coupon.affiliate_link && 'http://' != $coupon.affiliate_link}{$coupon.affiliate_link}{elseif $coupon.shop_affiliate_link && 'http://' != $coupon.shop_affiliate_link}{$coupon.shop_affiliate_link}{/if}">
	{if $coupon.featured}
	...
	<div class="pull-left">
		{if $coupon.coupon_image}
			...
			<a href="{printImage imgfile=$imgthumb.path url=true fullimage=true}" class="ia-item-thumbnail" rel="ia_lightbox">
				{printImage imgfile=$imgthumb.path title=$coupon.title|escape:'html' class='media-object' width=150}
			</a>
		{elseif $coupon.shop_image}
			<a href="{printImage imgfile=$coupon.shop_image.path url=true fullimage=true}" class="ia-item-thumbnail" rel="ia_lightbox">
				{printImage imgfile=$coupon.shop_image.path title=$coupon.shop_title|escape:'html' class='media-object' width=150}
			</a>
		{else}
...
$(function () {
	...
	$('#coupon-list-{$coupon.id} h4.media-heading a:first').on('click', function(e)
	{
		e.preventDefault();

		var affiliateLink = $('#coupon-list-{$coupon.id}').data('affiliate-link');
		var couponLink    = '{ia_url type='url' item='coupons' data=$coupon}';
