<div class="ia-item ia-item--border {if $listing.featured} ia-item--featured{/if}{if $listing.sponsored} ia-item--sponsored{/if}" data-affiliate-link="{if $listing.affiliate_link && 'http://' != $listing.affiliate_link}{$listing.affiliate_link}{elseif $listing.shop_affiliate_link && 'http://' != $listing.shop_affiliate_link}{$listing.shop_affiliate_link}{/if}" id="coupon-list-{$listing.id}">
    <div class="ia-item__image">
        ...
        {else}
            <a href="{$s="//api.webthumbnail.org?width=150&height=150&format=png&screen=1024&url={$listing.shop_website}" alt="Generated by WebThumbnail.org" class="img-responsive">
            </a>
        {/if}
        marty.const.IA_URL}shop/{$listing.shop_alias}.html" class="ia-item-thumbnail">
        <img src
    ...
    <div class="ia-item__content">
        ...
        <div class="ia-item__title">
            {ia_url type='link' item='coupons' data=$listing text=$listing.title} <small>{lang key='from'} <a href="{$smarty.const.IA_URL}shop/{$listing.shop_alias}.html">{$listing.shop_title|escape}</a></small>
        </div>
