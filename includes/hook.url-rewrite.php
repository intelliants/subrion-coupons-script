...
if ($iaView->url)
{
	...
	if ($alias && $iaDb->exists('`title_alias` = :alias AND `status` = :status', array('alias' => $alias, 'status' => iaCore::STATUS_ACTIVE), 'coupons_categories'))
	{
		$pageName = 'coupons_home';
		if ($pageUrl = $iaDb->one_bind('alias', '`name` = :page AND `status` = :status', array('page' => $pageName, 'status' => iaCore::STATUS_ACTIVE), 'pages'))
		{
